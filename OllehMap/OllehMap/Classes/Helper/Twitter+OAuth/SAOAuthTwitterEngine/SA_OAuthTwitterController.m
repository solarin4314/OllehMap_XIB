...
- (id) initWithEngine: (SA_OAuthTwitterEngine *) engine andOrientation:(UIInterfaceOrientation)theOrientation {
	if (self = [super init]) {
		...
		if (UIInterfaceOrientationIsLandscape( self.orientation ) )
			...
			_webView = [[UIWebView alloc] initWithFrame: CGRectMake(0, 44, 320,
...
- (void) loadView {
	...
	if ( UIInterfaceOrientationIsLandscape( self.orientation ) ) {
		...
		self.view = [[[UIView alloc]
                      initWithFrame: CGRectMake(0, 0, 320, [[UIScreen mainScreen] bounds].size.height - 20)]
                     autorelease];
		_backgroundView.frame =  CGRectMake(0, 44, 320, [[UIScreen mainScreen] bounds].size.height - 64);
	...
	UINavigationItem				*navItem = [[[UINavigationItem alloc] initWithTitle: NSLocalizedString(@"트위터 로그인", nil)] autorelease];
